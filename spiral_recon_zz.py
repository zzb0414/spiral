...
Spiral recon script.
Change log:
Modifying based of 3D cones. Zhibo. Started: 03/26/2026.
"""
import os
import numpy as np
import matplotlib.pyplot as plt

root_path = 'E:\\data\\python\\spiral\\'

# Plan to make calculating kspace trajectory, applying GIRF and calculating DCF as functions. Zhibo, 03/26/2026.
# kspace trajectory and density compensation function
...
# Should use UIHRawRead.run(fname, mode). Zhibo, 03/26/2026.
# Raw data
full_file_name = root_path + 'rawdata_cones_all.bin'
# ksp_finufft = np.reshape(ksp, [num_coils, -1])

# It is tricky when loading a binary like this. Should be more interpretable when using UIHRawRead.run(fname, mode). Zhibo, 03/26/2026.
num_coils = 11
size = os.path.getsize(full_file_name)
